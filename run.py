...
def tardqn(n_timesteps=num_iterations, learning_rate=learning_rate, gamma=gamma, policy="egreedy", epsilon=epsilon, environ=environ, tau=tau):
    global re
    env = gym.make(environ, max_episode_steps=1000)
    eval_env = gym.make(environ)
    rewards=[]
    agent = TarDQN(replay_buffer_max_length,
                        env.action_space.n, 
                        env.observation_space.shape[0],
                        epsilon=epsilon, 
                        gamma=gamma,
